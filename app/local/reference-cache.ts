...
export async function referenceStatuses(resultSetId: string, candidates: Candidate[]) {
...
export async function copyReferencePrompt(resultSetId: string, candidate: Candidate) {
  ...
  if (!status?.cached || status.reference !== "ready") throw new Error("请先点击右上角「下载代码」，缓存图片与代码后再复制提示词。");
  if (!status.cached.hasCode) throw new Error("此参考没有配套代码，无法复制含代码的绘图提示词。请选择有代码的参考。");
  await navigator.clipboard.writeText(status.cached.prompt);
}

export function referenceStatusText(status: ReferenceCacheStatus, candidate: Candidate) {
  const image = status.image === "local" ? "图片已在本地" : "精确图片待缓存";
  const reference = status.reference === "ready"
...
export function referenceStatusIcons(status: ReferenceCacheStatus, candidate: Candidate) {
  ...
  const state = codeReady ? "ready" : status.image === "local" ? "image" : "empty";
  const label = codeReady ? "图片与代码已缓存到本地" : status.image === "local" ? "图片已缓存到本地，代码未下载" : "图片与代码均未缓存到本地";
  const badge = element("span", undefined, `reference-status-light is-${state}`);
  const explanation = `${label}。${referenceStatusText(status, candidate)}`;
...
export async function cacheReferences(options: {
  ...
  try {
    ...
    if (!pending.length) { next.hidden = true; networkLabel.hidden = true; }
  ...
  next.onclick = async () => {
    ...
    next.onclick = async () => {
      ...
      status.textContent = `${completed} 个参考可用，本次执行 ${failed} 个失败。可用参考才能复制绘图提示词。`;
      options.onChanged();
    };
  };
}
