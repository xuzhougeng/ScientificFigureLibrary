...
Read these bundled references only when relevant; figure-library is the single Skill entrypoint.
- Before creating/updating template prose use [figure-description](figure-description/GUIDE.md).
...
- When the user actually requests adapting/replicating a selected template, use
  [figure-organization](figure-organization/GUIDE.md), then
  [figure-style](figure-style/GUIDE.md). Preserve reference fidelity by default.
...
`resultSetId`, `pageIndex`, `hasMore`, and opaque `nextCursor`. In the host or App,
`figure_library_search_page` receives only the current
...
After search, present the current page using the App or the host's image support.
Ordinary hosts can call `figure_library_get_candidate_images` with current-result
candidate IDs (at most 12), or read each candidate's `thumbnailUri`. Then wait
for user selection. Thumbnail browsing creates no exact-preview challenge or
receipt. Do not automatically inspect exact previews or walk every page.
Only explicit delegation to choose permits reviewing a small relevant subset.

## 5. Preview and confirm one exact candidate

The search result keeps model-visible `structuredContent` compact and carries
verified thumbnail Data URLs in result `_meta` for the App. Ordinary hosts use
the returned `thumbnailUri` or `figure_library_get_candidate_images` instead.
