...
import { defineOpenFigurePrOperations } from "./open-figure-pr-tools.ts";
import { definePublicationExportOperations } from "./publication-export-tools.ts";
import { defineProviderSourceOperations } from "./provider-source-tools.ts";
...
export async function createLibraryService(options: LibraryServiceOptions = {}) {
  ...
  const exactPreviewForConfirmation = async (options: {
    ...
    invocationSource: "app" | "headless" | "local";
    toolName: "figure_library_preview_exact" | "figure_library_preview_exact_headless" | "local_preview_exact";
  }): Promise<CallToolResult> => {
    const operationStartedAt = performance.now();
    ...
    try {
      ...
      const previewChallenge = previewConfirmations.issueChallenge({
        ...
        confirmationMode: options.invocationSource,
      });
      const responseEnvelope = outcome(
        ...
        options.invocationSource === "app"
          ...
          : options.invocationSource === "local"
            ? `Loaded the exact preview for ${preview.templateId}. Display this image in the local client and wait for an explicit user confirmation.`
      ...
      const structuredContent = {
        ...
        sha256: preview.sha256,
        previewSha256: preview.sha256,
        ...(options.invocationSource !== "app" ? { previewChallenge } : {}),
        ...(options.invocationSource === "local" ? { transportSha256: transport.transportSha256 } : {}),
      };
  ...
  const localOperations: LocalOperations = {
    preview: async (raw) => {
      const input = ExactPreviewInput.parse(raw);
      return exactPreviewForConfirmation({
        ...input, exactSelector: input.exactSelector as unknown as ExactTemplateSelector,
        invocationSource: "local", toolName: "local_preview_exact",
      });
    },
    confirm: async (raw) => {
      const input = z.object({
        previewChallenge: z.string().min(1).max(256),
        displayedImageSha256: z.string().regex(HASH),
        imageLoaded: z.literal(true), confirmedBy: z.literal("user"),
      }).strict().parse(raw);
      try {
        const receipt = previewConfirmations.confirm(input.previewChallenge, "local", input.displayedImageSha256);
        return terminal(outcome("ok", "preview_confirmed_local", "The user confirmed an exact preview in the local client. Review the materialization plan before applying.", "review_plan"), {
          previewReceipt: receipt.previewReceipt, confirmationMode: "local",
          resultSetId: receipt.resultSetId, providerId: receipt.providerId,
          exactSelector: receipt.exactSelector, previewSha256: receipt.previewSha256,
        });
      } catch (error) { return previewFailure("Local preview confirmation failed", error); }
    },
    library: async () => {
      const { versionedLibrary: library } = await currentLibraries();
      const items = [];
      for (const series of await library.listSeries()) {
        const head = series.workingHead ?? series.publishedHead;
        if (!head) continue;
        const content = await library.getContent(series.templateId, head.revisionId, head.contentDigest);
        if (!content) throw new Error("Library entry cannot resolve its exact content");
    ...
    asset: async (raw) => {
      const input = z.object({ templateId: z.string().min(1), revisionId: z.string().min(1),
        contentDigest: z.string().regex(HASH), logicalPath: z.string().min(1).max(4_000) }).strict().parse(raw);
      const { versionedLibrary: library } = await currentLibraries();
      const content = await library.getContent(input.templateId, input.revisionId, input.contentDigest);
      if (!content) throw new Error("Exact library content not found");
      const asset = content.assets.find((entry) => entry.logicalPath === input.logicalPath);
      if (!asset || asset.bytes > 16 * 1024 * 1024) throw new Error("Asset unavailable or exceeds local preview limit");
      const { bytes } = await library.readAsset(input);
      return terminal(outcome("ok", "local_asset_ready", "Read verified reference bytes without executing code."), {
  ...
  operations.defineResource(
    ...
    resourceTemplate("figure-library://diagnostics/{bundleId}"),
    {
  ...
  defineLibraryBindingOperations({ operations, runtime, workspaceRuntime, currentLibraries });
  defineLifecycleOperations({
    operations,
    currentLibrary: async () => (await currentLibraries()).versionedLibrary,
  });
  defineMaterializationOperations({
    operations,
    index,
    registry,
    moduleCatalogs: liveModuleCatalogs(),
...
export type LibraryService = Awaited<ReturnType<typeof createLibraryService>>;
