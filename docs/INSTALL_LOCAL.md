...
本地安装包不携带 FigureYa 与 Open Figure Modules 的图库缩略图、精确预览图，仅保留目录、许可和图片校验信息。绑定本机目录后，本地网页会提示一次性缓存这些预览图；打开“发现模板”可浏览全部图库图片，搜索则按关键词筛选。浏览或搜索当前页时下载缩略图，打开精确预览时下载对应图片。FigureYa 使用同一张参考图。

首次缓存或查看需要能访问 GitHub 的图片下载地址。若直连超时，可在“设置与连接”中填写本机回环 HTTP 代理，例如 `http://127.0.0.1:7890`；也可设置环境变量 `SFL_HTTPS_PROXY`（优先）或回环地址的 `HTTPS_PROXY`。代理只允许本机 loopback，使用 CONNECT 访问原来的 GitHub HTTPS 地址，下载后仍按 SHA-256 校验。下载校验成功后写入本机缓存，之后可离线复用；未缓存或下载失败的图片不能用于确认，恢复网络后重新缓存、搜索或打开预览即可重试。自己的图库图片继续从指定的本地图库读取。签名图片来源在“设置与连接”中手动添加：确认后会缓存该来源的目录快照，之后可再更新。
...
设置页的「图片缓存」显示缓存目录和占用空间，可复制路径或清除缓存。关闭 App 和使用它的 MCP 进程后，也可以直接删除该目录以释放空间；后续查看会重新下载。测试或高级配置可通过绝对路径 `SFL_PREVIEW_CACHE_DIR` 指定缓存目录。清除已确认图片的缓存后，需要重新预览再保存模板。
