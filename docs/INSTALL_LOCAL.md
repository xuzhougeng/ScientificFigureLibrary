...
3. 在“设置与连接”中确认全局图库和本地工作区的绝对目录。同一页可配置系统代理、查看图片缓存、添加已签名图库，或移除/恢复内置来源。本机已发布不能移除。默认不使用系统代理；若 Clash 等把 GitHub 解析到 198.18.x，请打开系统代理并填写本机回环地址（例如 `http://127.0.0.1:7897`）。
...
3. 在“设置与连接”中确认全局图库和本地工作区的绝对目录。同一页可配置系统代理、对某个图库缓存图片、添加已签名图库，或移除/恢复内置来源。
4. 使用“退出本地客户端”结束服务，再关闭页面。
...
本地安装包不携带 FigureYa 与 Open Figure Modules 的图库缩略图、精确预览图，仅保留目录、许可和图片校验信息。缓存必须由你主动触发某个图库：在设置的「图片缓存」或「图片来源」中对该图库执行「缓存图片」。打开「发现模板」可浏览全部图库图片，搜索则按关键词筛选。浏览或搜索当前页时下载缩略图，打开精确预览时下载对应图片；FigureYa 使用同一张参考图。安装、启动或勾选系统代理都不会批量下载全部图库。
...
设置页的「图片缓存」按图库列出可下载数量和已缓存数量，可对单个图库下载、复制路径或清除缓存。关闭 App 和使用它的 MCP 进程后，也可以直接删除该目录以释放空间；之后需要重新对某个图库缓存。测试或高级配置可通过绝对路径 `SFL_PREVIEW_CACHE_DIR` 指定缓存目录。清除已确认图片的缓存后，需要重新预览再保存模板。
