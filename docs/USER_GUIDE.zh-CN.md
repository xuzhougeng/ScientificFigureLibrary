...
在 macOS 客户端和 Windows/Linux 本地网页的图库卡片或详情页点击 ☆，即可收藏当前模板版本；★ 表示已收藏，再次点击可取消。侧边栏的「收藏」页面集中列出收藏，支持按名称、来源和用途搜索，并可打开预览或返回该模板的图库卡片继续使用。

收藏保存在本机用户配置目录，跨项目、重启后仍保留，清理图片或参考包缓存不会清空收藏。不同图库及不同精确版本分别记录。来源停用、模板撤下或版本更新导致原版本不可用时，打开会提示错误，记录仍保留，可取消收藏或从图库重新收藏新版本。收藏本身不下载参考包、不执行绘图，也不改变绘图参考的选择状态。
...
(离线)检查 Provider 状态。本地网页和 macOS 设置页也可列出清单地址、添加已签名图库，以及移除或恢复内置来源；本机已发布不能移除。若更新图库时报 `198.18.x` 非公网地址，在设置里打开系统代理并填写本机回环 HTTP 代理。
...
「连接外部图库」中的 FigureYa 和 Open Figure Modules 均提供「缓存图片」「缓存参考包」「更新缓存」。每次先展示当前目录的预览图/参考包数量及本地位置，确认后联网准备；缓存参考包只下载固定版本压缩包，不执行代码、不创建项目副本。图库卡片上的「参考包」状态对应这份整库缓存；复制提示词读取同一份参考包，不再另存展开副本。「更新缓存」校验并补齐当前固定版本的预览图和参考包；目录版本升级仍使用「检查更新」（FigureYa 随安装包更新）。失败项单独列出，可检查原因后重新生成计划。不同图库的缓存互不混合。保存到项目仍要精确预览确认。

本地 Web 客户端确认后关闭弹窗，在后台执行以上三种缓存操作；设置页的 FigureYa / Open Figure Modules「缓存图片」也使用同一流程。右下角「缓存任务」显示进度、当前条目及失败数量，可以折叠面板继续浏览，完成后可查看失败详情并收起任务。切换页面、刷新页面不会中断任务；退出本地服务会中断，重新启动后需再次发起缓存。同一图库已有运行任务时不会重复启动。

「我的图库」支持 Gallery 图片卡片和 List 紧凑列表，右上角可随时切换并记住上次选择。Gallery 按需加载当前草稿（没有草稿时为已发布版本）的预览图片；点击图片或「查看与管理」可继续审阅、更新和发布。List 保留资产名称、草稿/发布状态及管理入口。

在本地 App 的图库页面筛选图片，勾选最多 12 张后点击「复制绘图提示词」，或直接点击卡片上的复制按钮。复制前会检查本地参考包：已缓存且含代码的直接复制；尚未缓存时先下载固定版本再复制。浏览和勾选本身不会下载参考包；整库「缓存参考包」按固定版本获取压缩包，已有本地包可复用。详情中的「下载代码」在参考包未缓存时下载同一份压缩包。

卡片或图片详情中的「复制绘图提示词」会提供该参考实际图片、代码和说明文件的路径，以及输入要求和来源。将提示词交给有本机文件读取能力的 Agent；如果 AI 在远端或无法访问这些路径，上传提示词列出的文件后再作图。多选表示一次复制多份参考提示词，不是批量下达绘图任务。

卡片同时显示预览图和参考包状态：灰色为两者均未缓存，黄色为预览图已缓存、参考包尚未缓存，绿色为参考包已缓存。芯片文案只有「已缓存」或「待缓存」。悬停或键盘聚焦可读到具体说明。详情默认展开技术信息，右上角集中放置「下载代码」「复制提示词」「保存到项目」「复制图片」和关闭图标。无配套代码时说明原因，不复制含代码的提示词。预览图在 `indexes/preview-cache/`，参考包在 `source-packs/`，参考原件不要修改；项目适配请另存。设置中的预览图缓存与参考包是两类缓存。

macOS arm64 / x64 DMG 和 Windows x64 ZIP 不随包携带图库预览图。需要你在设置中对某个在线图库执行「缓存图片」；搜索该图库时仍会下载当前页缩略图，打开精确预览时下载对应图片。校验后缓存在用户目录，已缓存图片可离线查看。首次下载失败时恢复网络，再对该图库重新缓存或重新打开。设置页按图库显示可下载数量、占用空间，并可清除缓存；路径与架构选择见 [本地安装说明](INSTALL_LOCAL.md#图片按需下载)。
