...
macOS arm64 / x64 DMG 和 Windows x64 ZIP 不随包携带图库预览图。当前页缩略图和选中图片的精确预览按需下载，校验后缓存在用户目录；已缓存图片可离线查看。首次下载失败时恢复网络并重新打开。缓存位置、清理和架构选择见 [本地安装说明](INSTALL_LOCAL.md#图片按需下载)。
