...
「连接外部图库」中的 FigureYa 和 Open Figure Modules 均提供「缓存图片」「缓存代码」「更新缓存」。每次先展示当前目录的图片/源码包数量及本地位置，确认后联网准备；缓存代码不执行代码、不创建项目副本，也不替代使用某张参考前的精确预览确认。「更新缓存」校验并补齐当前固定版本的图片与代码；目录版本升级仍使用「检查更新」（FigureYa 随安装包更新）。失败项单独列出，可检查原因后重新生成计划。不同图库的缓存互不混合。
...
在本地 App 的图库页面筛选图片，勾选最多 12 张后点击「复制绘图提示词」，或直接点击卡片上的复制按钮。复制前会检查本地缓存：已缓存且含代码的参考直接复制；未缓存或缓存失效的会先查看精确图片、生成并核对缓存计划，确认后再缓存并复制。浏览和勾选本身不会下载代码包；代码按选定的固定版本获取，已有本地包可复用。详情中的「下载代码」仍可只缓存、不复制。
...
卡片同时显示预览图和参考包状态：灰色为预览图与参考包均未缓存，蓝色为图片可从本地图库读取但尚未写入预览缓存，黄色为预览图已缓存，绿色为精确图片与代码都已缓存。悬停或键盘聚焦可读到具体说明。详情默认展开技术信息，右上角集中放置「下载代码」「复制提示词」「保存到项目」「复制图片」和关闭图标。无配套代码时说明原因，不复制含代码的提示词。缓存目录在所选 Library 的 `indexes/reference-cache/`，参考原件不要修改；项目适配请另存。设置中的预览图缓存与这里的完整参考包是两类缓存。
