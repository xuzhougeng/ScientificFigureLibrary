...
本地客户端启动后会在后台检查官方最新稳定版；有新版时可点击「查看更新」前往发布页面，或点击「稍后」关闭提示。进入「设置 → 客户端更新」可查看当前版本并点击「检查更新」。网络错误会显示在设置页，可检查网络或代理后重试。更新需要在发布页面选择适合系统和处理器的安装包并手动安装；程序更新不会自动迁移或覆盖用户图库。
