...
「连接外部图库」中的 FigureYa 和 Open Figure Modules 均提供「缓存图片」「缓存参考包」「更新缓存」。每次先展示当前目录的预览图/参考包数量及本地位置，确认后联网准备；缓存参考包只下载固定版本压缩包，不执行代码、不创建项目副本，也不替代使用某张参考前的精确预览确认。图库卡片上的「参考包」状态对应这份整库缓存；复制提示词仍会在需要时确认精确图片并展开到本地。「更新缓存」校验并补齐当前固定版本的预览图和参考包；目录版本升级仍使用「检查更新」（FigureYa 随安装包更新）。失败项单独列出，可检查原因后重新生成计划。不同图库的缓存互不混合。
...
在本地 App 的图库页面筛选图片，勾选最多 12 张后点击「复制绘图提示词」，或直接点击卡片上的复制按钮。复制前会检查本地缓存：已展开且含代码的参考直接复制；参考包已整库缓存但尚未展开，或缓存失效时，会先查看精确图片、生成并核对缓存计划，确认后再展开并复制。浏览和勾选本身不会下载参考包；整库「缓存参考包」按固定版本获取压缩包，已有本地包可复用。详情中的「下载代码」仍可只展开、不复制。
...
卡片同时显示预览图和参考包状态：灰色为预览图与参考包均未缓存，蓝色为图片可从本地图库读取但尚未写入预览缓存，黄色为预览图已缓存，绿色为精确图片已展开且可复制。参考包芯片在整库压缩包落地后显示「已缓存」，展开后显示「可复制 · 含代码」。悬停或键盘聚焦可读到具体说明。详情默认展开技术信息，右上角集中放置「下载代码」「复制提示词」「保存到项目」「复制图片」和关闭图标。无配套代码时说明原因，不复制含代码的提示词。整库参考包在 `source-packs/`，展开后的精确参考在 `indexes/reference-cache/`，参考原件不要修改；项目适配请另存。设置中的预览图缓存与参考包是两类缓存。
