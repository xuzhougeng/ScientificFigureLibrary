...
在本地 App 的图库页面筛选图片，点击「缓存此参考」，或勾选最多 12 张后点击「缓存所选参考」。查看精确图片、生成并核对缓存计划，再确认下载。浏览和勾选本身不会下载代码包；代码按选定的固定版本获取，已有本地包可复用。

缓存完成后，卡片或图片详情中的「复制绘图提示词」会提供该参考实际图片、代码和说明文件的路径，以及输入要求和来源。将提示词交给有本机文件读取能力的 Agent；如果 AI 在远端或无法访问这些路径，上传提示词列出的文件后再作图。多选表示批量缓存，不是批量下达绘图任务。

卡片和详情的状态灯表示本地准备情况：灰色为图片与代码均未缓存，黄色为仅图片在本地，绿色为图片与代码都已缓存。悬停或键盘聚焦可读到具体说明。详情默认展开技术信息，右上角集中放置「下载代码」「复制提示词」「保存到项目」「复制图片」和关闭图标。复制提示词前会重新校验本地代码；未下载或缓存损坏时要求先下载，无配套代码时说明原因。缓存目录在所选 Library 的 `indexes/reference-cache/`，参考原件不要修改；项目适配请另存。设置中的预览图缓存与这里的完整参考包是两类缓存。
