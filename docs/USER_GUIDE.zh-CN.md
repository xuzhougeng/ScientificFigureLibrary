...
在图库或「我的图库」的卡片/详情点击「编辑标签」，填写逗号分隔的标签后保存。支持中文和多个标签，清空后保存可移除；原有模板标签不变。使用「自定义标签」下拉框筛选，图库可同时限定来源和搜索词。每个图片条目最多 20 个标签，每个最多 40 字符。标签属于个人分类，按已绑定图库隔离并跟随同一来源的同一条目，程序升级后保留；现有图库备份不包含这些本机配置。保存冲突时重新打开编辑器，查看最新状态后再修改。存储和迁移范围见 [本地客户端说明](LOCAL_CLIENT.md#图片自定义标签)。
