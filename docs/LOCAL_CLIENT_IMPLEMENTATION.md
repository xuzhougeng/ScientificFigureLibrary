...
本地服务绑定 loopback 的动态端口。浏览器通过一次性启动凭据建立同源会话，SwiftUI 通过私有启动通道取得凭据。界面 API 按操作白名单直接调用共享业务；不开放通用 shell 或任意文件下载。本地图片确认使用独立的 local 模式，保留精确资产和传输图片身份。本地界面多选用于复制绘图提示词；未缓存的参考会先确认后缓存，再复制。不创建 Agent 任务或轮询 Agent 状态。

## 本地参考缓存（开发中版本）

网页与 SwiftUI 图库卡片同时显示预览图与参考包状态。外部图库的「缓存参考包」下载固定版本压缩包，与卡片「参考包」为同一类缓存。选择参考后的主操作是复制绘图提示词：已展开且含代码的参考直接复制；否则先查看精确预览、生成计划并确认，再展开对应固定版本并复制。浏览和勾选本身不展开参考包。提示词来自实际校验过的文件清单，包含图片、代码、说明文件的绝对路径、输入要求和精确来源身份。AI 无法读取本机路径时需上传文件；缓存不表示绘图已执行。

参考副本位于用户选择的 Library 的 `indexes/reference-cache/`，属于派生缓存，不创建 Local Published Release。状态读取以原有权威材料化 receipt 和完整文件清单重新校验；缺失、损坏或版本变化不会继续作为可用参考交付。重新获取使用新的目录，不覆盖旧文件。图片预览缓存与完整参考缓存独立；「清除缓存」仍只清除设置页标明的在线预览图。

批量操作逐项保留精确预览、一次性 receipt、plan/apply 和重放规则，分别显示失败与成功，不承诺事务性全部成功。macOS 实现需在 macOS 上编译与人工验收；Windows 的构建及测试不能替代该验证。

详情默认展开技术信息，右上角统一放置本地状态灯、下载代码、复制提示词、保存到项目、复制图片和关闭图标。状态灯以灰色表示都未缓存、蓝色表示图片可本地读取、黄色表示预览图已缓存、绿色表示图片与代码都已缓存，悬停和键盘聚焦可读取文字说明。复制提示词前重新校验参考文件；未缓存时先确认精确图片并缓存再复制，无配套代码时说明原因，不复制含代码的提示词。
