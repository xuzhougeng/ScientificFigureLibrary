...
- macOS 拆分为 arm64 / x64，分别只携带本架构的 SwiftUI、MCP 启动器和 Node；Windows 继续使用内置 Node 和本地网页。
...
- 本地网页与 macOS 设置页显示按需预览缓存目录和占用，并按图库列出可下载预览；用户必须对某个图库执行「缓存图片」才会批量下载，安装或启动不会预取。两边都列出图库来源。可添加已签名图库；FigureYa / Community / Open Figure Modules 可从搜索中移除并恢复；本机已发布不能移除。
