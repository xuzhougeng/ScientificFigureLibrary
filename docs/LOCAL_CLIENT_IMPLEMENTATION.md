...
本地服务绑定 loopback 的动态端口。浏览器通过一次性启动凭据建立同源会话，SwiftUI 通过私有启动通道取得凭据。界面 API 按操作白名单直接调用共享业务；不开放通用 shell 或任意文件下载。本地图片确认使用独立的 local 模式，保留精确资产和传输图片身份。本地界面多选用于批量缓存参考包，不创建 Agent 任务或轮询 Agent 状态。

## 本地参考缓存（开发中版本）

网页与 SwiftUI 提供单张及最多 12 张参考的缓存入口。浏览只获取所需图片；明确选择后查看精确预览、生成计划并确认，才获取对应固定版本的代码包。缓存成功后，卡片或详情提供「复制绘图提示词」，内容来自实际校验过的文件清单，包含图片、代码、说明文件的绝对路径、输入要求和精确来源身份。AI 无法读取本机路径时需上传文件；缓存不表示绘图已执行。
...
详情默认展开技术信息，右上角统一放置本地状态灯、下载代码、复制提示词、保存到项目、复制图片和关闭图标。状态灯以灰色表示都未缓存、黄色表示仅图片在本地、绿色表示图片与代码都已缓存，悬停和键盘聚焦可读取文字说明。复制提示词前重新校验参考文件；代码未缓存时提示先下载，无配套代码时说明原因，不复制含代码的提示词。
