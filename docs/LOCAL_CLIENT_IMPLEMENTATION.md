...
- [ ] Windows 包内 Node、独立启动入口及各宿主插件配置；无系统 Node 的验证。
- [ ] macOS SwiftUI：与网页相同的核心知识库流程、原生窗口和目录选择。
- [ ] macOS 私有 Node 运行时、App 构建与安装产物；目标系统验证。
...
- 开发环境：Linux/WSL2；当前未发现 Swift 编译器。Windows 系统路径可见，实际互操作待测试。
- 尚未开始原生或未预装 Node 的目标环境验收。
...
- macOS SwiftUI、Windows 内置 Node 分发、目标系统安装以及更完整的管理交互仍未完成；上述验证不代表整个 App 交付完成。
