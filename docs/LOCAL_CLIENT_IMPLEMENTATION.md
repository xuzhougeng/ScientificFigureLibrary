...
网页与 SwiftUI 图库卡片同时显示预览图与参考包状态。选择参考后的主操作是复制绘图提示词：已校验通过且含代码的参考直接复制；否则先查看精确预览、生成计划并确认，再缓存对应固定版本的代码包并复制。浏览和勾选本身不下载代码包。提示词来自实际校验过的文件清单，包含图片、代码、说明文件的绝对路径、输入要求和精确来源身份。AI 无法读取本机路径时需上传文件；缓存不表示绘图已执行。
