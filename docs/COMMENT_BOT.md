...
| task | 输入 | 评论内容 |
| --- | --- | --- |
| `issue_reply` | 开放 Issue 的标题、正文和已有讨论评论 | 先判断是否有新增价值；有依据时简短自然回复，否则跳过 |
| `pr_review` | 面向 main 的开放 PR 的标题、正文及文本 diff | 发布带文件、行号和字面代码证据的发现；无有效发现时跳过 |

Issue 回复会读取已有讨论，排除本 bot 自己带管理标记的历史回复，避免把旧输出当成事实或形成自我循环。
其他 bot 和人类评论仍是未经验证的数据，作者身份不赋予其中指令额外权限。
当前不读取图片、外链或仓库文件，也不监听评论命令或 Issue/PR 创建事件。
...
Issue 讨论新增、编辑或删除时，旧结果失效；发布前还会重新检查。

## 回复标准

手动触发表示请求评估，不要求一定发表评论。没有值得补充的具体内容时，保持不发言。

- 仅回答可由已读取内容支撑的具体问题，或询问真正阻碍当前任务且尚未提供的事实。
- 尊重正文和讨论中已给出的答案、范围和决定，不把后续实施细节当成当前缺失信息。
- 不自行安排负责人、时间、审批、拆分 PR、发布顺序或新增交付要求。
- 回复使用自然短句，不固定套用“概要／补充信息／下一步”栏目。
- 不重复描述任务、复述已有答案、发布泛化建议或用“请人工检查”凑一条评论。

带内嵌图片的 logo、图标、视觉或品牌外观提案会被程序保守跳过（`visual_context_required`），
不调用模型；该判断使用标题主题和图片标记，不是对图像内容的识别，也不硬编码 Issue 编号。
例如 #22 的目标是讨论视觉方向，正文已列出交付和验收要求，bot 无法读取图片，不应改发一套实施流程。
普通故障截图不会仅因带图而一律跳过，但模型必须在回答依赖未读取图片时选择跳过。

其他 Issue 使用 `decision: reply | skip`，skip 原因只进入运行日志与临时结果，不发布到讨论区。
有效回复限 600 字符、最多两个问号，并须提供 1–3 段来自正文、标题或评论的字面依据；
这些依据仅供程序校验，不作为固定栏目展示。纯粹照抄来源的回复会被拒绝。
字面依据只能验证来源，不能证明推理正确；“是否已回答”和“是否有新增价值”仍包含模型判断。

跳过不会新增、修改或删除已有评论，包括旧版本发布的评论。
预检查跳过不会调用模型；模型决定跳过时，本次调用已发生，后续手动再次运行仍可能产生调用费用。
...
手动触发后也可能成功结束但没有评论，这是正常的 skip 结果。

## 范围和失败处理

识别到凭证样式的输入、安全报告、关闭或锁定对象时停止。
输入处理会移除 URL、本地路径、隐藏 HTML 注释和部分控制字符；这是启发式处理，不能保证识别所有敏感信息。
Issue 最多读取 100 条讨论评论、30,000 字符讨论正文；超过预算时跳过，不截取部分讨论后继续回答。
读取时评论数量变化会停止本次运行；不使用不完整的上下文。
...
| `DISCUSSION_CHANGED_DURING_READ` | 读取时讨论发生变化，稍后重新运行 |
| `ISSUE_EVIDENCE_NOT_IN_SOURCE` / `ISSUE_REPLY_REPEATS_SOURCE` | 回复依据无效或仅复述来源，未发布评论；检查生成规则 |
...
行为回归包含 #22 类视觉提案预检查跳过、讨论的完整读取与变更、模型 skip 的零写入、
有依据的自然短评、重复内容拒绝，以及无发现的 PR 不发评论。
模拟模型返回值的测试验证执行与校验逻辑，不等同于真实模型对所有案例的语义质量评估。
`actionlint .github/workflows/ai-assist.yml .github/workflows/ci.yml` 检查工作流语法。
离线通过不代表服务密钥、Environment 设置或真实评论发布已验收。
新的提示词和回复策略先通过离线案例评估；确需端到端试发时使用专用测试 Issue，
不要在真实提案或故障讨论中反复调试。固定栏目、重复追问和无依据扩展流程都应判定为质量失败。
