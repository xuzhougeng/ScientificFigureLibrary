...
状态库损坏时会保留带时间戳的 `.corrupt-*` 文件并创建干净数据库；这不会删除或重写任何图片、源码包或 Library 资产。
...
本地参考使用流程为：检索/浏览 → 选择参考并复制绘图提示词。卡片同时显示预览图和参考包的本地状态。复制前会检查缓存；已校验通过且含代码的参考直接复制，未缓存的会先查看精确图片、确认缓存计划并写入本地，再复制提示词。多选表示一次复制多份参考提示词，不是为每个参考下达绘图任务。提示词列出真实缓存文件路径；不能读取本机文件的 AI 需要用户上传材料。原有「保存到项目」保留为独立导出流程。整库预览图或源码包仍在「连接外部图库」中按图库缓存。
