...
本地参考使用流程为：检索/浏览 → 选择单张或多张参考 → 查看精确图片并确认缓存计划 → 缓存图片与代码 → 复制单张参考的绘图提示词。卡片分别显示图片的本地状态和参考包状态，已有校验通过的参考可直接复用。多选只用于批量缓存，不表示请求为每个参考生成一张图。提示词列出真实缓存文件路径；不能读取本机文件的 AI 需要用户上传材料。原有「保存到项目」保留为独立导出流程。
