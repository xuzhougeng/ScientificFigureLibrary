...
本地参考使用流程为：检索/浏览 → 选择参考并复制绘图提示词。卡片同时显示预览图和参考包的本地状态。整库「缓存参考包」下载固定版本压缩包；卡片「参考包 · 已缓存」表示该压缩包已在本地。「可复制 · 含代码」表示已确认精确图片并展开。复制前会检查缓存；已展开且含代码的参考直接复制，否则先查看精确图片、确认缓存计划并写入本地，再复制提示词。多选表示一次复制多份参考提示词，不是为每个参考下达绘图任务。提示词列出真实缓存文件路径；不能读取本机文件的 AI 需要用户上传材料。原有「保存到项目」保留为独立导出流程。整库预览图或参考包仍在「连接外部图库」中按图库缓存。
