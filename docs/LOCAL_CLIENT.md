...
状态：2026-09-15 用户确认的产品定义与平台方案；本文列出的原生客户端、独立网页和
内置运行时尚未实现。现有代码状态见 [Agent 接入方案](AGENT_INTEGRATION.md)。
...
当前插件配置仍使用系统可调用的 `node` 命令；免预装 Node 是待实现的分发能力。

## 现有 UI 的复用方式

[现有界面入口](../app/main.ts) 通过 `App.connect()`、宿主工具调用与
`updateModelContext` 接收候选、预览及交接。它尚不能直接作为独立网页运行。
...
外部 Agent 的机制，需在实现时补充明确契约和测试；不能伪造 Agent 看图或 App 加载事实。
...
这些连接细节属于待实现内容，不能用普通静态 HTML 文件代替。
...
本次仅统一产品定义和架构文档，不代表以上客户端已完成开发或可以下载安装。
