...
状态：0.8.1 提供 arm64 / x64 独立 macOS 包与按需下载预览；已实现原生 SwiftUI 客户端、独立本地网页和内置 Node 的打包入口，作为预览版分发。
...
- `no-node` 版移除运行时，使用相同的已构建后端与界面，启动前检查本机 Node.js 22+；无需安装 npm 依赖。macOS 支持选择 Node 可执行文件，Windows 支持 PATH 和 `SFL_NODE_BINARY`。
