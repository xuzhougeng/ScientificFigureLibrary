...
const environment = () => ({ AI_ENABLED: "true", AI_API_KEY: fakeKey,
...
test("approved API URL/model and protocol match the user's exact choices", () => {
  const settings = client.loadSettings(policy, environment());
  assert.equal(settings.endpoint, "https://api.jadelai.top/v1/chat/completions");
  assert.equal(settings.model, "gemini-3.8-flash-high");
  assert.equal(client.loadSettings(policy, { ...environment(), AI_BASE_URL: policy.baseUrl + "/" }).endpoint, settings.endpoint);
});

for (const url of ["http://api.jadelai.top/v1", "https://evil.example/v1", "https://localhost/v1",
  "https://user:password@api.jadelai.top/v1", "https://api.jadelai.top/v1?token=x", "https://api.jadelai.top/v1/chat/completions"]) {
  test(`reject unapproved endpoint ${url}`, () => {
    assert.throws(() => client.loadSettings(policy, { ...environment(), AI_BASE_URL: url }));
  });
}

test("disabled/missing-key/different-model/unsupported-protocol are fail closed", () => {
  for (const override of [{ AI_ENABLED: "false" }, { AI_API_KEY: "" }, { AI_MODEL: "different-model" }, { AI_API_STYLE: "responses" }]) {
...
test("client sends no tools or unsupported optional generation parameters", async () => {
  ...
  const output = await client.requestModel(inputFor(), policy, environment(), { fetch: async (url, options) => {
    calls++;
    assert.equal(url, policy.baseUrl + "/chat/completions");
...
test("PR input is pinned to both base and head, never executes a patch", async () => {
  const { github, state } = mockGitHub();
  const input = await assist.collectInput({ github, context: contextFor(), task: "pr_review", number: 23 });
  ...
  const updated = await assist.collectInput({ github, context: contextFor(), task: "pr_review", number: 23 });
...
test("file-list truncation and missing text patches cannot produce an all-clear", async () => {
  ...
  await assert.rejects(assist.collectInput({ github, context: contextFor(), task: "pr_review", number: 23 }), /PR_FILE_LIST_INCOMPLETE/);
  state.pr.changed_files = 1;
  delete state.files[0].patch;
  await assert.rejects(assist.collectInput({ github, context: contextFor(), task: "pr_review", number: 23 }), /TEXT_PATCH_MISSING/);
});
...
test("patch hunk and API line counts must be complete", async () => {
  ...
  await assert.rejects(assist.collectInput({ github, context: contextFor(), task: "pr_review", number: 23 }), /PATCH_TRUNCATED/);
});
...
test("source changes during paginated reading are rejected", async () => {
  ...
  await assert.rejects(assist.collectInput({ github, context: contextFor(), task: "pr_review", number: 23 }), /PR_CHANGED_DURING_READ/);
});

test("wrong object type and PR base are rejected", async () => {
  const { github, state } = mockGitHub();
  await assert.rejects(assist.collectInput({ github, context: contextFor(), task: "issue_reply", number: 23 }), /KIND_MISMATCH/);
  state.pr.base.ref = "another-branch";
  await assert.rejects(assist.collectInput({ github, context: contextFor(), task: "pr_review", number: 23 }), /PR_BASE_OR_STATE/);
});
...
test("redacted lines cannot be mistaken for literal source-code evidence", async () => {
  ...
  const prepared = await assist.collectInput({ github, context: contextFor(), task: "pr_review", number: 23 });
