...
function loadSettings(policy, env) {
  ...
  const baseUrl = (env.AI_BASE_URL || policy.baseUrl).replace(/\/$/, "");
  let parsed;
  try { parsed = new URL(baseUrl); } catch { throw new SafeError("INVALID_API_ENDPOINT"); }
  requireValue(parsed.protocol === "https:" && !parsed.username && !parsed.password && !parsed.search && !parsed.hash, "INVALID_API_ENDPOINT");
  requireValue(baseUrl === policy.baseUrl, "API_ENDPOINT_NOT_APPROVED");
  const model = env.AI_MODEL || policy.model;
  requireValue(model === policy.model, "MODEL_NOT_APPROVED");
...
module.exports = { SYSTEM_RULES, loadSettings, readLimited, requestModel };
